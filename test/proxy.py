...
def Get_proxy_ip():
    ...
    req = Request(r'http://www.xicidaili.com/nn/', headers=headers)
    response = urlopen(req)  #request.urlopen(req)
...
def Proxy_read(proxy_list, user_agent_list, i):
    ...
    print proxy_ip
    # print '当前代理ip：%s' % proxy_ip.decode()
    ...
    proxy_support = ProxyHandler({'http':proxy_ip})
